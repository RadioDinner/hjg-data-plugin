...
console.log("[32] Hourly work on a mentor build (summary, stub, CSV, standing rate, 9961)");
{
  ...
  // The emailed PDF (lib/payStubPdf) carries the same hourly lines and total.
  const pdfDoc = mentorStubPdfDoc(stub);
  const pdfTable = (pdfDoc.content[2] as { table: { body: unknown[][] } }).table;
  eq(
    pdfTable.body.length,
    1 + 1 + 1 + 2 + 1,
    "PDF rows = header + mentee + piece + 2 hourly + TOTAL",
  );
  const pdfTotal = pdfTable.body[pdfTable.body.length - 1] as { text?: string }[];
  eq(pdfTotal[5]?.text, pdfUsd(stub.totals.payout), "PDF TOTAL = revenue share + piece + hourly");
  const pdfJson = JSON.stringify(pdfDoc.content);
  eq(pdfJson.includes("Training delivery"), true, "hourly line printed in the PDF");
  eq(pdfJson.includes("PIECE WORK + HOURLY"), true, "PDF shares one card for both extras");
  eq(pdfJson.includes("HJG adjustments"), false, "PDF: extras are never 'adjustments'");
  eq(pdfJson.includes("Hourly work is paid in full"), true, "PDF fine print explains hourly");
  const hourlyPdf = await pdfmake.createPdf(pdfDoc).getBuffer();
  assert(hourlyPdf.length > 1000, `hourly mentor PDF renders (${hourlyPdf.length} bytes)`);
  ...
  // Saving on a pre-9961 database: hours carry data → names 9961; none → saves.
  ...
  for (const withHours of [false, true]) {
    ...
    if (withHours) {
      eq(saved.length, 0, "pre-9961 + hourly work → refuses to save without it");
      eq(err.includes("9961_payout_build_hours.sql"), true, "…and names 9961");
    } else {
      eq(saved.length, 1, "pre-9961 + no hourly work → saves");
      eq("hour_items" in (saved[0] ?? {}), false, "…without the hourly columns");
    }
  }
}

console.log("[33] Mentor pay cost by month — piece work + hourly as a cost (computeMentorPayCost)");
{
