...
- It counts as a **cost on the Margins tab**, together with piece work: see *Mentor pay cost by month (§608)*. Needs migration \`9962_payout_build_hours.sql\`.
...
- Hourly work needs migration \`9962_payout_build_hours.sql\`. The card warns you until it's applied.

### Source
- \`ca_invoices\`, \`ca_appointments\` and \`ca_engagements\` (the same rules as *Margins on Mentoring*), plus \`payout_builds\` (\`piece_items\`, \`hour_items\`, \`hourly_rate\`, \`status\`). The math is \`computeMentorPayCost\` in \`lib/margins.ts\`, verified in \`scripts/verify-metrics.ts\` §32.`,
  },
