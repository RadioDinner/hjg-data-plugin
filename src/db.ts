...
import { type StageBasis } from "../lib/journey";
...
export interface PayoutBuildRecord {
  ...
  // 9962; reads degrade gracefully when unapplied). hourlyRate = the default $/h
...
export async function fetchPayoutBuilds(): Promise<Map<string, PayoutBuildRecord>> {
  ...
  // 9962) are read when present and default when absent, so ANY subset of those
...
export interface MentorCostData {
  ...
  hourlyColumn: boolean; // false = migration 9962 not applied (hourly work can't be saved yet)
}
