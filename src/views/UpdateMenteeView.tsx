...
function currentEngagement(engs: ClientEngagement[], today: string): ClientEngagement | null {
  const live = engs.filter(
    (e) =>
      !e.isCanceled && e.startDate && e.startDate <= today && (!e.endDate || e.endDate >= today),
  );
  if (live.length)
...
export function UpdateMenteeView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchCompanyOptions().catch(() => ({}) as Record<string, string>),
    ])
      .then(([m, c, opts]) => {
        if (!live) return;
        setRows(m);
  ...
  const loaded = useMemo(
    () => effective.find((m) => m.id === loadedId) ?? null,
    [effective, loadedId],
  );
  ...
  const currentCoach =
    current?.coachId != null ? (coachNames.get(current.coachId) ?? `#${current.coachId}`) : null;
    ...
    <div className="stack">
      ...
      >
        <div className="muted" style={{ fontSize: 13, marginTop: -2 }}>
          Forms that change a mentee's state. The first form is <strong>Transition Mentee</strong>:
          load a mentee to see their current (from) state, then pick what they're transitioning to.
          Recording the transition is the next build phase — today this is the wiring.
        </div>
      </CollapsibleCard>

      <CollapsibleCard
        id="updateMentee.transition"
        title="Transition Mentee"
        sectionId="updateMentee.transition"
      >
        {/* Picker: search + select + Load. */}
        <div
          className="filter-bar"
          style={{ padding: "4px 0 12px", borderBottom: "1px solid var(--line)" }}
        >
          ...
          >
            ...
            <div className="card card--inset">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 16px" }}>
                ...
                <FromField
                  label="CA client id"
                  value={loaded.clientId != null ? String(loaded.clientId) : "not linked to CA"}
                />
                ...
                <FromField
                  label="First meeting"
                  value={loaded.firstMeeting ? fmtDate(loaded.firstMeeting) : null}
                />
                <FromField
                  label="Last meeting"
                  value={loaded.lastMeeting ? fmtDate(loaded.lastMeeting) : null}
                />
                <FromField label="Meetings" value={String(loaded.meetingCount)} />
                <FromField
                  label="Discovery"
                  value={loaded.discoveryDate ? fmtDate(loaded.discoveryDate) : null}
                />
              </div>
              ...
              {loaded.clientId == null ? (
                <p className="muted" style={{ margin: 0 }}>
                  Not linked to a CA client — no engagement data.
                </p>
              ) : engError ? (
                ...
                <p className="muted" style={{ margin: 0 }}>
                  No un-canceled engagements on record.
                </p>
              ) : (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 16px" }}>
                  ...
                  <FromField
                    label="Start"
                    value={current.startDate ? fmtDate(current.startDate) : null}
                  />
                  <FromField
                    label="End"
                    value={
            ...
            <div className="card card--inset">
              ...
              <p className="muted" style={{ fontSize: 12 }}>
                ...
                <strong>Company options (§451) → Update Mentee → “Transition to… options”</strong>.
              </p>
              {transitionTo && (
                <p style={{ fontSize: 14 }}>
                  <strong>{loaded.name}</strong>: {loaded.currentStage ?? loaded.statusLabel} →{" "}
                  <strong>{transitionTo}</strong>
                </p>
              )}
              <button
