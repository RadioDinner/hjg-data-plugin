...
const STAGES: {
  ...
  label: string;
  colorIndex: number;
  reached: (m: EffectiveMentee) => string | null;
}[] = [
...
function pillClass(m: EffectiveMentee): string {
  const s =
    m.effectiveStatus ??
    (m.caStatus === "graduated" ? "graduated" : m.caStatus === "active" ? "active" : "inactive");
...
export function MenteesView() {
  ...
  const conflictCount = useMemo(
    () => effective.filter((m) => !m.isTest && (m.conflicts.length > 0 || m.coarseExit)).length,
    [effective],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return effective.filter((m) => {
      ...
      if (
        ...
        !(m.ownerCoachName ?? "").toLowerCase().includes(q)
      )
        return false;
      return true;
    });
        ...
        >
          ...
          {r.conflict ? (
            <span
              className="pill"
              style={{ marginLeft: 6 }}
              title="CA / Notion / hand disagree, or a coarse Notion exit to classify"
            >
              ⚠
            </span>
          ) : null}
      ...
      format: (r: Row) => {
        ...
        return (
          <select
            ...
            value={m.status ?? ""}
            onChange={(e) =>
              inlineSave(m.id, { status: (e.target.value || null) as MenteeMgmtStatus | null })
            }
            title="Hand-zone status (wins over Notion/CA). Blank = use the auto-derived value."
          >
            <option value="">
              {m.status == null ? `— ${m.statusLabel} (auto) —` : "— Unclassified —"}
            </option>
  ...
  const tableRows: Row[] = filtered.map((m) => ({
    ...
    stage: m.currentStage
      ? (STAGE_FILTERS.find((s) => s.value === m.currentStage)?.label ?? m.currentStage)
      : "—",
  ...
  async function addMentee() {
    const name = window.prompt(
      "New mentee name (a hand-added prospect, not yet in CoachAccountable):",
    );
  ...
  function onImported(r: NotionImportResult) {
    ...
    const amb = r.ambiguous.length
      ? ` · ${r.ambiguous.length} ambiguous (skipped: ${r.ambiguous.map((a) => a.name).join(", ")})`
      : "";
  ...
  const conflictFields = useMemo(
    () => new Set((selectedEff?.conflicts ?? []).map((c) => c.field)),
    [selectedEff],
  );
  type SourceRow = {
    field: string;
      ...
      ? [
          {
            field: "name",
            label: "Name",
            ca: selectedRow.ca_name,
            notion: selectedRow.notion_name,
            eff: selectedEff.name,
            acceptCa: () => setDraftField("name_override", selectedRow.ca_name ?? ""),
            acceptNotion: () => setDraftField("name_override", selectedRow.notion_name ?? ""),
          },
          {
            field: "coach",
            label: "Coach",
            ca: selectedRow.ca_owner_coach_name,
            notion: selectedRow.notion_coach,
            eff: selectedEff.ownerCoachName,
            acceptCa: () => setDraftField("coach_override", selectedRow.ca_owner_coach_name ?? ""),
            acceptNotion: () => setDraftField("coach_override", selectedRow.notion_coach ?? ""),
          },
          {
            field: "email",
            label: "Email",
            ca: null,
            notion: selectedRow.notion_email,
            eff: selectedEff.email,
            acceptNotion: () => setDraftField("email_override", selectedRow.notion_email ?? ""),
          },
          {
            field: "phone",
            label: "Phone",
            ca: null,
            notion: selectedRow.notion_phone,
            eff: selectedEff.phone,
            acceptNotion: () => setDraftField("phone_override", selectedRow.notion_phone ?? ""),
          },
          {
            field: "discoveryDate",
            label: "Discovery date",
            ca: fmtDate(selectedRow.ca_discovery_date) || null,
            notion: fmtDate(selectedRow.notion_dc_date) || null,
            eff: fmtDate(selectedEff.discoveryDate) || null,
            acceptCa: () => setDraftField("discovery_date_override", selectedRow.ca_discovery_date),
            acceptNotion: () =>
              setDraftField("discovery_date_override", selectedRow.notion_dc_date),
          },
          {
            field: "status",
            label: "Status",
            ca: selectedRow.ca_status,
            notion: selectedRow.notion_status,
            eff: selectedEff.statusLabel,
            acceptNotion: () => {
              const m = mapNotionStatus(selectedRow.notion_status);
              if (m) setDraftField("status", m.status);
            },
          },
        ]
      : [];
  ...
  return (
    <div className="view">
      <div className="view__header">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button
            ...
            title="Import a Notion Mentees Database CSV into the Notion zone (matched by name; never clobbers CA or your edits)"
          >
            Import Notion CSV
          </button>
      ...
      <p className="view__hint">
        HJG's <strong>source of truth</strong> for every mentee. Each row blends three zones — a{" "}
        <strong>CA zone</strong> (refreshed from CoachAccountable every sync), a{" "}
        <strong>Notion zone</strong> (imported from your Notion export), and a{" "}
        <strong>hand zone</strong>
        (your edits, never overwritten). The table shows the <strong>effective</strong> value (hand
        wins, then Notion, then CA). Edit <strong>Status</strong>, <strong>Coach</strong> and{" "}
        <strong>Discovery</strong> right in the grid, or click a name to open the full editor on the
        right. Funnel &amp; exits now live on the <strong>Metrics</strong> tab.
      </p>
          ...
          >
            <div className="journey-filters">
              ...
              <label
                ...
                title="Only mentees where CA / Notion / hand disagree, or a coarse Notion exit needs classifying"
              >
                <input
                  type="checkbox"
            ...
            {loading ? (
              ...
              <SortableTable
                ...
                exportName="mentees"
                maxRows={500}
                emptyText="No mentees match the filters."
              />
            )}
          </CollapsibleCard>
        </div>
          ...
          {selectedRow && selectedEff ? (
            <CollapsibleCard
              ...
              title={
                <>
                  {selectedEff.name}{" "}
                  <span className={pillClass(selectedEff)}>{selectedEff.statusLabel}</span>
                </>
              }
              actions={
            ...
            >
              <p className="view__hint" style={{ marginTop: 4 }}>
                Coach: <strong>{selectedEff.ownerCoachName ?? "—"}</strong> ·{" "}
                {selectedEff.meetingCount} meetings ·{" "}
                {selectedRow.client_id != null
                  ? `CA client #${selectedRow.client_id}`
                  : "Notion-only (no CA client)"}{" "}
                · CA synced: {selectedRow.ca_synced_at ? fmtDate(selectedRow.ca_synced_at) : "—"} ·
                Notion imported:{" "}
              ...
              {selectedEff.coarseExit && (
                <div className="notice notice--warn" style={{ marginBottom: 8 }}>
                  Notion records this exit coarsely (<strong>{selectedEff.notionStatus}</strong>).
                  Classify it precisely below — set the real status (quit / no mentoring / fired /
                  declined) and the exit stage.
                </div>
              )}
              {selectedEff.notionCoachConflict && (
                <div className="notice notice--warn" style={{ marginBottom: 8 }}>
                  Notion's <strong>Mentor 1</strong> and <strong>Mentor</strong> disagree for this
                  mentee — they should match. Set the coach by hand below.
                </div>
              )}
                ...
                {STAGES.map((s) => {
                  ...
                  return (
                    <div key={s.key} className="mentee-rail__node">
                      <span
                        className="mentee-rail__dot"
                        style={{ background: d ? color : "transparent", borderColor: color }}
                      />
              ...
              <h3 style={{ marginBottom: 6 }}>
                Data sources — CoachAccountable · Notion · your edits
              </h3>
                  ...
                  <tbody>
                    {sources.map((s) => {
                      ...
                      return (
                        <tr
                          key={s.field}
                          style={conflict ? { background: "rgba(245,158,11,0.10)" } : undefined}
                        >
                          <td>
                            {s.label}
                            {conflict ? (
                          ...
                          <td>
                            ...
                            {s.ca && s.acceptCa ? (
                              <button
                                ...
                                style={{ marginLeft: 6 }}
                                onClick={s.acceptCa}
                                title="Copy into your hand edit"
                              >
                                → hand
                              </button>
                            ) : null}
              ...
              <div className="mentee-detail__grid">
                ...
                <div>
                  <h3 style={{ marginTop: 0 }}>
                    Your edits (the source of truth — wins over Notion + CA)
                  </h3>
                  <div className="form-grid">
                    ...
                    <label className="form-field">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setDraftField(
                            "status",
                            (e.target.value || null) as MenteeMgmtStatus | null,
                          )
                        }
                      >
                    ...
                    <label className="form-field">
                      ...
                      <select
                        value={draft.status_stage ?? ""}
                        onChange={(e) => setDraftField("status_stage", e.target.value || null)}
                      >
                    ...
                    <label className="form-field">
                      ...
                      <input
                        ...
                        onChange={(e) => setDraftField("status_date", e.target.value || null)}
                      />
                    </label>
                    {DATE_FIELDS.map((f) => (
                        ...
                        <span>
                          ...
                          {f.ca ? (
                            <span className="muted">
                              (CA: {fmtDate(selectedRow[f.ca] as string | null) || "—"})
                            </span>
                          ) : null}
                        </span>
                        <input
                    ...
                    <label className="form-field">
                      ...
                      <input
                        value={(draft.coach_override as string) ?? ""}
                        placeholder={selectedEff.ownerCoachName ?? ""}
                        onChange={(e) => setDraftField("coach_override", e.target.value)}
                      />
                    </label>
                    <label className="form-field">
                      <span>Email (override)</span>
                      <input
                        value={(draft.email_override as string) ?? ""}
                        onChange={(e) => setDraftField("email_override", e.target.value)}
                      />
                    </label>
                    <label className="form-field">
                      <span>Phone (override)</span>
                      <input
                        value={(draft.phone_override as string) ?? ""}
                        onChange={(e) => setDraftField("phone_override", e.target.value)}
                      />
                    </label>
                    ...
                    <label className="form-field">
                      ...
                      <span>
                        <input
                          ...
                          onChange={(e) => setDraftField("is_test", e.target.checked)}
                        />{" "}
                        ...
                        <tbody>
                          {engagements.map((e, i) => (
                            <tr key={i}>
                              ...
                              <td>
                                {e.isCanceled ? "canceled" : e.isComplete ? "complete" : "open"}
                              </td>
                            </tr>
                          ))}
                        </tbody>
